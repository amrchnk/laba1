...
namespace FileExchanger
{
    ...
    public partial class MainPage_admin : Form
    {
        ...
        {
            InitializeComponent();
            this.doings.DataSource = activity;
        }
        ...
        {
            string answer = con.GetJSON($"admins?login={this.lgn.Text}");
            if (answer != "[]")
            {
        ...
        private void Save_Click(object sender, EventArgs e)
        {
            ...
            foreach(var item in this.doings.Text)
            {
                doings = doings + item + "\n";
            }
            ...
            foreach (Control control in report.Controls)
            {
                if (control is RadioButton)
                {
                    ...
                    if(k==0) MessageBox.Show("Выберите формат файла из предложенных");
                }
            }
